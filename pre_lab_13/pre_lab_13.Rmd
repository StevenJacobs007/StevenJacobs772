...
**Answer** I am seeing a dataframe with all 50 states, listing their statistics regarding their education spending per pupil, percentage growth in GDP, percentage change in population, and the winner of the states popular vote in the 2020 election.
```{r}
...
**Answer** Based off the pattern, I would say that their is a somewhat recognizable correlation between a states population change and GDP growth. The states with a low GDP seem to also have a low population change, and vice versa.
```{r}
...
**Answer** This plot has dots that are much more spaced out, which makes me want to say that there is not a strong relation between a states GDP growth and education spending per pupil.
```{r}
...
**Answer** Basically confirming what I said earlier, the relationship between GDP growth and spending per pupil is negative, which means it isn't a strong relationship. Relationship between GDP growth and population change is positive though, showing a somewhat stronger relationship. 
...
**Answer** I see our correlation which is shown in the code above, which I believe is the r value. It is also giving us a P value.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** I see our r value again which matches our code in task 5. Also see another P value.
```{r}
...
**Answer** I see the comparison between Biden and Trumps average GDP growth, spending per pupil, and population growth. Biden winning all but population growth.
```{r}
state_data |>
  group_by(vote_2020) |>
  summarise(
...
**Answer** I see the two averages regarding population change and voting numbers, which are shown above. We have a t value as well as p value again.
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** This is a test run regarding per pupil spending. the p value is extremely low. We can see that there is probably a pattern to take note of regarding spending on education and the amount of voters for a presidential candidate.
```{r}
