...
# Calculate the percentage of votes Cox received within each county for 2022
md_gov_county <- md_gov_county %>%
  mutate(Cox_percentage = (Cox / rowSums(select(., starts_with("Cox")), na.rm = TRUE)) * 100)
...
md_gov_county_18 <- md_gov_county_18 %>%
  mutate(Hogan_percentage = (Hogan / rowSums(select(., starts_with("Hogan")), na.rm = TRUE)) * 100)

# View the resulting data frame
print(md_gov_county_18)
```
...
**A2.** 
I feel like the story here is the staggering amount of mail-ins during the 2020 election...Understanding that the pandemic took place as its peak during this year, there can be a story developed on how many people who usually dont mail in their votes, ultimatley had no other choice but to do so in order to have their voice heard during the 2020 election... 

https://datawrapper.dwcdn.net/29nSW/1/
...
# Group by zip code and calculate total calls and percentage
result_df <- montgomery_2022_overdoses %>%
  group_by(zip) %>%
  summarise(
    total_calls = n(),
    percentage_of_total_calls = (n() / nrow(montgomery_2022_overdoses)) * 100
  )
...
output_file <- file.path(Sys.getenv("HOME"), "Downloads", "result_data.csv")

# Save the dataframe as a CSV file using write_csv
write_csv(result_df, path = output_file)

# Confirm that the CSV file has been created
cat("CSV file saved successfully:", output_file, "\n")
...
**A3.**
https://datawrapper.dwcdn.net/kJaJx/1/

I think we can find a story regarding the top zip codes in the county... Possibly finding an angle to see why those specific zip codes attract the most calls... Get in contact with the police department to see if there is an excess amount of drugs being sent in, and find a possible correlation.
