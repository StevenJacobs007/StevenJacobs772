...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** There are duplicate names, or multiple versions rather of one name
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer** you can see that it grouped together duplicates... overall it just cleaned up the entries... such as certain commas in Grantee names, like FAIR HILL FARM, INC turned into FAIR HILL FARM INC
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer** Walden Sierra is consolidated into one entry...Washington County Community Action Council had multiple entries and now its consolidated... it changed WAYNE M HOLLAND to M WAYNE HOLLAND which I find interesting
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** Honestly, it made some minimal changes compared to the previous, some were good, but not to the point where I can say its the better option...
