...
**Task** Load the four CSV population estimate files. Do they have the same column structure? **Answer** Yes, they have the same number of columns, and they all have the same names for the columns.
...
It gave us a new file, with 96 rows, all of the dataframes combined!
...
**Task** Run the following code to filter on Frederick County's registration data and look at the results. What stands out to you about the changes in party registration over time? **Answer** It doesnt provide a total for 2022 registered voters. I also notice that the democrats had the biggest yearly increases, compared to the year by year for the Republicans. 
...
**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** There are 24 rows...There should be 24 rows
...
**Task** Create a new dataframe with population and voter data joined together. Add a description of what this code does to your reference notebook. It gives us every registered voter by county total, along with their political identification... Also gives us the total population by county.
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer** It breaks us down the registered voters by race, as well as a column letting us know how many of those registered voters are 18 or older.
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** Prince George's County has the highest percentage of 18 or older year olds that are black. 60.9%. And 77% are Democrats in P.G. County
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** Overall, there is a much less population of 18+ Asians compared to Blacks in Maryland. Howard County has the most Asians over 18 percentage with 19.4%. With 52.6% of them Democrats.
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N column? Its in numeric data, <dbl> **Answer**
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N? **Answer** After matching it with the joined column its a character datatype now <chr>
