...
**Answer** I think it is because it only gives us the Journalism School courses instead of all the courses
...
**Answer** It gives us a much larger breakdown of the courses than AND does, AND barriers it much more
...
**Answer:** it actually appears as its listed by date... it says <date> istead of <chr>
...
**Answer** I am assuming that are all the first date of the month, just because it allocates for all of the expenditures in the month as a whole... If it listed it by date, we would only June 2022 dates for a very long time
...
**Answer** It is taking together all of the votes, but separately, so we can look at a percentage breakdown by nominee instead of just the number of votes itself. 
...
**Answer** it is giving the percentage in a decending order now
...
**Answer** Yes because I can see the number of votes each person received by the county, and what percentage that is for Moore, to that state compared to everyone else
...
**Answer** I used CHATGPT like suggested, and it gave me some great answers they are posted below:

In the tidyverse, which is a collection of R packages for data manipulation and visualization, you can achieve similar functionality to `str_to_upper` using various functions. Two such functions are `mutate` and `str_to_upper`, which are part of the `dplyr` and `stringr` packages, respectively. Here's a description of what each of these functions does:

1. `mutate` from `dplyr`:
   - `mutate` is used for creating new columns or modifying existing ones in a data frame. While it doesn't directly convert strings to uppercase, you can use it in combination with other functions to achieve that effect.
   - Example: You can use `mutate` with `toupper` from the base R package to convert a column of strings to uppercase.
   ...
   # Convert the 'Name' column to uppercase using mutate and toupper
   df <- df %>%
...
Another tidyverse function that can be used to achieve similar functionality to `str_to_upper` is `str_to_uppercase` from the `stringr` package. Here's a description of what it does:

2. `str_to_uppercase` from `stringr`:
   - `str_to_uppercase` is a function from the `stringr` package that, like `str_to_upper`, is specifically designed to convert character vectors to uppercase.
   - Example:
   ...
   # Convert the character vector to uppercase using str_to_uppercase
   uppercase_names <- str_to_uppercase(names)

   # Resulting character vector
   # [1] "JOHN"  "ALICE" "BOB"
   ```

`str_to_uppercase` and `str_to_upper` are similar in that they both perform case conversion on character vectors, making them uppercase. However, `str_to_uppercase` is a part of the `stringr` package, which is known for its user-friendly string manipulation functions, while `str_to_upper` is a part of the same package but provides a slightly different naming convention for consistency with other stringr functions.
   ```
...
**Answer** I believe that it designating that each state name in contained in its own column, and then arranging it with the value we are looking for. Showing the state by the space
...
**Answer** I think so, its showing me the states that were represented with an address column
...
1. Maybe we would use mutate to find unit conversions? or maybe to find the ratio between the values of two columns?

2.  You could ask to use filtering to detect the common value, or maybe, just to break down the data by the decade maybe?

3. You can ask to find the earliest date detected for a specific person or value, and ask for the latest date submitted in the data, and ask for a comparison of the two and maybe report changes or similarities. 
