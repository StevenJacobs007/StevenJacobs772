...
This code starts by creating a new data frame or variable called baltco_911_calls_by_month.
It uses the pipe operator (|>) to pass the baltco_911_calls data frame to the next operation.
mutate(month = month(date, label=TRUE)) |>:

In this step, a new column called "month" is added to the baltco_911_calls data frame.
The values in the "month" column are calculated using the month() function on the "date" column, and the label=TRUE argument indicates that the month names should be used.
The result is then passed to the next operation.
group_by(month) |>:

Here, the data is grouped by the "month" column, meaning the data is organized into separate groups for each unique month.
This step is useful for performing calculations within each month.
summarize(total_calls = n()) |>:

The data is summarized within each group, and a new column called "total_calls" is created.
The n() function is used to count the number of rows in each group, which essentially counts the number of 911 calls for each month.
The result is then passed to the next operation.
arrange(desc(total_calls)):

Finally, the data is arranged or sorted in descending order based on the "total_calls" column.
This step orders the months from the one with the highest number of calls to the one with the lowest.
baltco_911_calls_by_month:

This line is not part of the data manipulation process but is used to display or store the resulting data frame with the monthly 911 call information.
...
**Answer** All I see is a clear white box??
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** I see a basic chart... X axis represents the month and the Y axis represents the count of calls within each month...
...
**Answer** It gives us the same info but it is somewhat overlapping... The bottom axis is somewhat 'smushed'
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? 
**Answer** We flipped the axis location to allow us for more space with out total calls by the month... X axis is now Y axis but vice versa...
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? 
**Answer** It removes the gray background... 
...
**Answer** It has an appealing color for a background... gives us a decent visual, much more easier to look at...
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? 
**Answer** We now have titles, along with a source, there is a grid line for each bar to keep things organized...
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer** I see a certain amount of dates wehre calls were made... correspondent to the amount of calls made on that specific date. 
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer** I see a line plot now... basically showing the frequency of calls within a month...
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** Super un-organized.... All of the month dates on the X axis are completely cluttered...
...
**Task** Run the following code. Explain what you see after running the code in the space below.\
**Answer** It is still cluttered at the bottom, but much easier to correspond... Also much more spaced out
