...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer** In the columns I see zip codes, shape length and shape area, a geometry column, object id column, and an existing name column
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer** It contains the 526 zip codes mentioned above, in the respective columns mentioned above. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** It gives us a cloropeth map... I would assume divided up based on zip code.
...
**Task** Run the following code. Describe the output in the space below. **Answer** it just tells me it downloaded 354 KB.
...
**Task** Run the following code. Describe the output in the space below. **Answer** It is  giving me locations, correlated to names, county it resides in, geoid's, county FP's... etc. along with a min and max.
...
**Task** Run the following code. Describe the output in the space below. **Answer** It is giving me an error
...
**Task** Run the following code. Describe the output in the space below. **Answer** Closer viewed clorepeth... Not as dissected as the one earlier, bigger divisions...
...
**Task** Run the following code. Describe the output in the space below. **Answer** the boundaries are dissected and divided much much more... Broken down by zipcode and X and Y axis I would assume. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Gives me a dataframe with 527 rows of zip codes and the correlated amount of foreclosures made in March 2023
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer** It addes the foreclosure numbers to the corresponding zip codes based off the earlier dataset.
...
**Task** Run the following code. Describe the output in the space below. **Answer** It gives us color to our map... And with this we are able to have a much better visual presentation of the foreclosure fequency by zip code on this map... 
...
**Task** Run the following code. Describe the output in the space below. **Answer** We have 63 rows and 27 columns of the Maryland counties, with a corresponding zip code, and 5-year ACS, but the margin of error is N/A.
...
**Task** Run the following code. Describe the output in the space below. **Answer** Now, I see the county table we made above joined together with the foreclosure data by county... Still N/A's in the MOE, but the rest is there... 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Here I see a date, with a corresponding type of foreclosure, along with the corresponding county, and the mount of foreclosure notices the specific county received by that date of March 1, 2023...
...
**Task** Run the following code. Describe the output in the space below. **Answer** It renamed the name coulumn to county, and removed Maryland to make it lowercase..  I also do not see the N/A's anymore... population name is changed to estimate.. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** It is a cleaned up version of what we made before... titles are lowercase, foreclosure type is directly next to the amount of notices now... the population of the county is directly next to the county name now... 
...
**Task** Run the following code. Describe the output in the space below. **Answer** I see pretty much the same but with different counties than above... Much much more quantities of notices than the group above... 
...
**Task** Run the following code. Describe the output in the space below. **Answer** we have a cloropeth, but the labels are in black lettering which is honestly difficult to read I would imagine for most people... But it is showing us the rate of notices made within each county determined by the shade of blue within the county represented. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** We changed the colors to magma colors, which makes it look much cooler, as well as providing us with a much better visual to look at. Better to analyze to the naked eye than those shades of blue in the map above...
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** With this map, we can see or almost make a correlation that neighboring counties share almost the same rate of foreclosures, as there are many instances where the neighboring counties are the same shade of magma color!
