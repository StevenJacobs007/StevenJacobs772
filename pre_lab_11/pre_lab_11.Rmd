...
**Answer** It gives us 4 columns: a date, title which describes the release, URL column which has the link to the releases, and text which gives a more indepth description... 
...
**Answer** It is pulling out words that are sometimes spelled with an uppercase, and other times with a lowercase... It is also pulling uneccesary punctuation marks within the text... 
...
**Answer** It is breaking down all of the words in our original text column. I would say that it is paraphrasing and/or removing the descriptions within the text. All of the words that appear multiple times, and worth noting. 
...
**Answer** It is definitely interesting to see the amount of times certain words are used. Describing the chart, you can see that words such as "the" "and" & "to" are the most occurrences, which seems pretty likely. I find it weird that "Maryland" is towards the bottom of the list. Obviously used more than the ones that did not make the list, but I would just expect it to be more unique as far as ranking.
...
**Answer** For sure much more interesting... Not nearly as many common words, more so words that are directly relevant to the subject at hand. 
...
**Answer** I notice that "health" is in the top three of all of the lists. "Communities" makes all three lists. 
...
**Task** Run the following code and describe the results.
**Answer** A lot of the top ten phrases have to do with the pandemic, which is much expected as it first occurred in 2021. A lot of the conversation must have been ways to get back to somewhat normalcy after the peak of the outbreak. 
...
**Task** Run the following code and describe the results.
**Answer** I still notice that "covid-19" makes the list. As well as "health care". But, more of the phrases in my opinion are more related to locations than the 2021 list did. Maybe because these locations are implementing programs or more so setting the framework for projects that will get these locations back on their two feet post-covid.
...
**Task** Run the following code and describe the results.
**Answer** Covid-19 is no longer in the top 10! "Health care" is still present though.
...
**Answer** It is letting us know which top-words are rather considered positive or negative.. In my opinion, it seems like to me that the descriptions of positive or negative are worthy... Seems like for the most part the top three to five words for each list are mostly considered positive, which is good to see.
