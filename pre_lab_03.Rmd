...
**Answer** It has parsing issues, tells us to call problems() on your data frame. When you run the problems it makes a table of all the parsing problems
...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** No, no warning on parsing issue
...
**Answer** Vendor name is a character <chr>  Amount is a number <dbl>.  Date is a character <chr> as well
...
**Answer** There are 549 zip codes that dont have 5 characters. Maybe because another form of geographical identification was presented such as the state.
...
**Answer** summarize grouped our output by Fiscal Year, group_by let us know whether it was a state grant or loan, and the count tells us how much. A potential problem is that there is some N/A's in the category data, not letting us know if it was a grant or loan...
...
**Answer** I see a grantor, associated by: grantee, zip code, Fiscal Year, Amount, small description, Grant Category(?), Fiscal Period, and date
...
**Answer** it changed Zipe Code to zip_code, and it removed capital letters, along with replacing spaces with underscores...
...
**Answer** Its making sure that using group_by will result in fewer rows due to the inconsistent capitalization...
...
**Answer** There are at least 50 possible duplicates
...
**Answer** it has 17,740 rows in the new dataframe below
...
**Answer** It gives us the first five digits of every zip code on the left,, and gets rid of anything after using mutate
...
**Answer** 21202 has the most amount of money from the Maryland Tourism Board. That zip code is Baltimore, Maryland. That would make a lot of sense as Baltimore, Maryland is one of the top tourist city attractions in the state. Many tourists from all over the world tend to make a stop in Baltimore, Maryland to see the sights to offer after spending time in Washington DC.
